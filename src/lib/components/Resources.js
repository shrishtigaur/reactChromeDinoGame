import React from 'react';
import ReactDom  from 'react-dom';
import Batman_1x from "../../img/Batman/1x-BatmanCharSprite.png"
import Batman_2x from "../../img/Batman/2x-BatmanCharSprite.png"
import Mickey_1x from "../../img/MickeyNew/MickeyUpdated1x.png"
import Mickey_2x from "../../img/MickeyNew/MickeyUpdated2x.png"
import Dino_1x from "../../img/Dino&Sonic/1x-trex.png"
import Dino_2x from "../../img/Dino&Sonic/2x-trex.png"
import Sonic_1x from "../../img/Dino&Sonic/1x-sonic.png"
import Sonic_2x from "../../img/Dino&Sonic/2x-sonic.png"
class Recources extends React.Component {
  constructor(props){
    super(props)
    this.state={
      status:true
    }
  }
  HandleState(){
    this.setState({status:false})
  }
      render() {   
        document.body.onkeyup=(e)=>{
          if(e.keyCode===32 || e.keyCode===38){
            this.HandleState()
          }
        }
        document.body.addEventListener=('click',(event) => {
          this.HandleState()
      });
      let _1x_obstacle_large=""
      let _1x_obstacle_small=""
      let _1x_cloud=""
      let _1x_text=""
      let _1x_horizon=""
      let _1x_player=""
      let _1x_restart=""
      let _2x_obstacle_large=""
      let _2x_obstacle_small=""
      let _2x_cloud=""
      let  _2x_text=""
      let _2x_horizon=""
      let _2x_player=""
      let _2x_restart=""
      if(this.props.playerdetails==="Sonic"){
     _1x_player=Sonic_1x;
     _2x_player=Sonic_2x;
     _1x_obstacle_large=require('../../img/Dino&Sonic/1x-obstacle-large.png');
     _1x_obstacle_small=require('../../img/Dino&Sonic/1x-obstacle-small.png');
     _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
     _1x_horizon=require('../../img/Dino&Sonic/1x-horizon.png')
     _1x_text=require('../../img/Dino&Sonic/1x-text.png')
     _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
     _2x_obstacle_large=require('../../img/Dino&Sonic/2x-obstacle-large.png')
     _2x_obstacle_small=require('../../img/Dino&Sonic/2x-obstacle-small.png')
     _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
     _2x_text=require('../../img/Dino&Sonic/2x-text.png')
     _2x_horizon=require('../../img/Dino&Sonic/2x-horizon.png')
     _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
    
   }
      else if(this.props.playerdetails==="Mickey"){
        _1x_player=Mickey_1x;
        _2x_player=Mickey_2x
        _1x_obstacle_large=require('../../img/MickeyNew/objlarge1x.png');
        _1x_obstacle_small=require('../../img/MickeyNew/1x-obstacle-small.png');
        _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
        _1x_horizon=require('../../img/MickeyNew/floor sprite.png')
        _1x_text=require('../../img/Dino&Sonic/1x-text.png')
        _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
        _2x_obstacle_large=require('../../img/MickeyNew/objlarge2x.png')
        _2x_obstacle_small=require('../../img/MickeyNew/2x-obstacle-small.png')
        _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
        _2x_text=require('../../img/Dino&Sonic/2x-text.png')
        _2x_horizon=require('../../img/MickeyNew/floor sprite2x.png')
        _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
      }
      else if(this.props.playerdetails==="Aladdin"){
        _1x_player=require('../../img/Aladdin/1x-trex.png')
        _2x_player=require('../../img/Aladdin/2x-trex.png')
        _1x_obstacle_large=require('../../img/MickeyNew/objlarge1x.png');
        _1x_obstacle_small=require('../../img/MickeyNew/1x-obstacle-small.png');
        _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
        _1x_horizon=require('../../img/MickeyNew/floor sprite.png')
        _1x_text=require('../../img/Dino&Sonic/1x-text.png')
        _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
        _2x_obstacle_large=require('../../img/MickeyNew/objlarge2x.png')
        _2x_obstacle_small=require('../../img/MickeyNew/2x-obstacle-small.png')
        _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
        _2x_text=require('../../img/Dino&Sonic/2x-text.png')
        _2x_horizon=require('../../img/MickeyNew/floor sprite2x.png')
        _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
      }
      else if(this.props.playerdetails==="JackSparrow"){
        _1x_player=require('../../img/Jack Sparrow/1x-trex.png');
        _2x_player=require('../../img/Jack Sparrow/2x-trex.png');
        _1x_obstacle_large=require('../../img/Jack Sparrow/1x-obstacle-large.png');
        _1x_obstacle_small=require('../../img/Jack Sparrow/1x-obstacle-small.png');
        _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
        _1x_horizon=require('../../img/MickeyNew/floor sprite.png')
        _1x_text=require('../../img/Dino&Sonic/1x-text.png')
        _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
        _2x_obstacle_large=require('../../img/Jack Sparrow/2x-obstacle-large.png')
        _2x_obstacle_small=require('../../img/Jack Sparrow/2x-obstacle-small.png')
        _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
        _2x_text=require('../../img/Dino&Sonic/2x-text.png')
        _2x_horizon=require('../../img/MickeyNew/floor sprite2x.png')
        _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
      }
     
      
     else if(this.props.playerdetails==="Batman"){
      _1x_player=Batman_1x;
      _2x_player=Batman_2x
      _1x_obstacle_large=require('../../img/Batman/1x-objlarge.png');
      _1x_obstacle_small=require('../../img/Batman/1xsmallObjects.png');
      _1x_cloud=require('../../img/Batman/Cloud.png')
      _1x_horizon=require('../../img/Batman/Floor2.png')
      _1x_text=require('../../img/Dino&Sonic/1x-text.png')
      _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
      _2x_obstacle_large=require('../../img/Batman/2x-objlarge.png')
      _2x_obstacle_small=require('../../img/Batman/2x-obstaclesmall.png')
      _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
      _2x_text=require('../../img/Batman/2x-text.png')
      _2x_horizon=require('../../img/Batman/floor sprite2x.png')
      _2x_restart=require('../../img/Batman/2x-restart.png')
      }  
       else if(this.props.playerdetails==="Dino"){
        _1x_player=Dino_1x
        _2x_player=Dino_2x
        _1x_obstacle_large=require('../../img/Dino&Sonic/1x-obstacle-large.png');
        _1x_obstacle_small=require('../../img/Dino&Sonic/1x-obstacle-small.png');
        _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
        _1x_horizon=require('../../img/Dino&Sonic/1x-horizon.png')
        _1x_text=require('../../img/Dino&Sonic/1x-text.png')
        _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
        _2x_obstacle_large=require('../../img/Dino&Sonic/2x-obstacle-large.png')
        _2x_obstacle_small=require('../../img/Dino&Sonic/2x-obstacle-small.png')
        _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
        _2x_text=require('../../img/Dino&Sonic/2x-text.png')
        _2x_horizon=require('../../img/Dino&Sonic/2x-horizon.png')
        _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
    }

    else if(this.props.playerdetails==="BlackRanger"){
      _1x_player=require('../../img/Power rangers/1xcharblack.png');
      _2x_player=require('../../img/Power rangers/2xcharblack.png');
      _1x_obstacle_large=require('../../img/Power rangers/1x-obstacle-large-sheet.png');
      _1x_obstacle_small=require('../../img/Power rangers/1xsmallobj.png');
      _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
      _1x_horizon=require('../../img/Mickey/floor sprite.png')
      _1x_text=require('../../img/Dino&Sonic/1x-text.png')
      _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
      _2x_obstacle_large=require('../../img/Power rangers/2x-obstacle-large-sheet.png')
      _2x_obstacle_small=require('../../img/Power rangers/2xsmallobj.png')
      _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
      _2x_text=require('../../img/Dino&Sonic/2x-text.png')
      _2x_horizon=require('../../img/Mickey/floor sprite2x.png')
      _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
    }

    else if(this.props.playerdetails==="GreenRanger"){
      _1x_player=require('../../img/Power rangers/1xchargreen.png');
      _2x_player=require('../../img/Power rangers/2xchargreen.png');
      _1x_obstacle_large=require('../../img/Power rangers/1x-obstacle-large-sheet.png');
      _1x_obstacle_small=require('../../img/Power rangers/1xsmallobj.png');
      _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
      _1x_horizon=require('../../img/Mickey/floor sprite.png')
      _1x_text=require('../../img/Dino&Sonic/1x-text.png')
      _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
      _2x_obstacle_large=require('../../img/Power rangers/2x-obstacle-large-sheet.png')
      _2x_obstacle_small=require('../../img/Power rangers/2xsmallobj.png')
      _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
      _2x_text=require('../../img/Dino&Sonic/2x-text.png')
      _2x_horizon=require('../../img/Mickey/floor sprite2x.png')
      _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
    }

    else if(this.props.playerdetails==="RedRanger"){
      _1x_player=require('../../img/Power rangers/1xchared.png');
      _2x_player=require('../../img/Power rangers/2xchared.png');
      _1x_obstacle_large=require('../../img/Power rangers/1x-obstacle-large-sheet.png');
      _1x_obstacle_small=require('../../img/Power rangers/1xsmallobj.png');
      _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
      _1x_horizon=require('../../img/Mickey/floor sprite.png')
      _1x_text=require('../../img/Dino&Sonic/1x-text.png')
      _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
      _2x_obstacle_large=require('../../img/Power rangers/2x-obstacle-large-sheet.png')
      _2x_obstacle_small=require('../../img/Power rangers/2xsmallobj.png')
      _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
      _2x_text=require('../../img/Dino&Sonic/2x-text.png')
      _2x_horizon=require('../../img/Mickey/floor sprite2x.png')
      _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
    }
    else if(this.props.playerdetails==="Godzilla"){
      _1x_player=require('../../img/Godzilla/1x-trex.png')
      _2x_player=require('../../img/Godzilla/2x-trex.png')
      _1x_obstacle_large=require('../../img/Godzilla/1x-obstacle-large.png');
      _1x_obstacle_small=require('../../img/Godzilla/1x-obstacle-small.png');
      _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
      _1x_horizon=require('../../img/Mickey/floor sprite.png')
      _1x_text=require('../../img/Dino&Sonic/1x-text.png')
      _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
      _2x_obstacle_large=require('../../img/Godzilla/2x-obstacle-large.png')
      _2x_obstacle_small=require('../../img/Godzilla/2x-obstacle-small.png')
      _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
      _2x_text=require('../../img/Dino&Sonic/2x-text.png')
      _2x_horizon=require('../../img/Mickey/floor sprite2x.png')
      _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
  }
  else if(this.props.playerdetails==="Mario"){
    _1x_player=require('../../img/Mario/1xMarioCharacter.png')
    _2x_player=require('../../img/Mario/2xMarioCharacter.png')
    _1x_obstacle_large=require('../../img/Mario/1x-obstacle-large-export.png');
    _1x_obstacle_small=require('../../img/Mario/1x-obstacle-small-export.png');
    _1x_cloud=require('../../img/Dino&Sonic/1x-cloud.png')
    _1x_horizon=require('../../img/Mickey/floor sprite.png')
    _1x_text=require('../../img/Dino&Sonic/1x-text.png')
    _1x_restart=require('../../img/Dino&Sonic/1x-restart.png')
    _2x_obstacle_large=require('../../img/Mario/2x-obstacle-large-export.png')
    _2x_obstacle_small=require('../../img/Mario/2x-obstacle-small.png')
    _2x_cloud=require('../../img/Dino&Sonic/2x-cloud.png')
    _2x_text=require('../../img/Dino&Sonic/2x-text.png')
    _2x_horizon=require('../../img/Mickey/floor sprite2x.png')
    _2x_restart=require('../../img/Dino&Sonic/2x-restart.png')
}
        return (
      <React.Fragment>
        {this.state.status? <h3 style={{marginTop:'60px',color:'green'}}>Please Hit Spacebar/UpKey to Start....</h3>:''}
          <div id="offline-resources">
        <div id="offline-resources-1x">
        <img id="1x-obstacle-large" src={_1x_obstacle_large}/>
        <img id="1x-obstacle-small" src={_1x_obstacle_small}/>
        <img id="1x-cloud" src={_1x_cloud}/>
      <img id="1x-text" src={_1x_text}/>
        <img id="1x-horizon" src={_1x_horizon}/>
        <img id="1x-trex" src={_1x_player}/>
        <img id="1x-restart" src={_1x_restart}/>
      </div>
      <div id="offline-resources-2x">
        <img id="2x-obstacle-large" src={_2x_obstacle_large}/>
        <img id="2x-obstacle-small" src={_2x_obstacle_small}/>
        <img id="2x-cloud" src={_2x_cloud}/>
        <img id="2x-text" src={_2x_text}/>
        <img id="2x-horizon" src={_2x_horizon}/>
        <img id="2x-trex"   src={_2x_player}/>
        <img id="2x-restart" src={_2x_restart}/>
      </div>

                <template id="audio-resources">
                  <audio id="offline-sound-press" src="data:audio/mpeg;base64,"></audio>
                  <audio id="offline-sound-hit" src="data:audio/mpeg;base64,"></audio>
                  <audio id="offline-sound-reached" src="data:audio/mpeg;base64,"></audio>
                </template>
              </div>
              
    </React.Fragment>
        )
       
    }
  }

export default Recources;
